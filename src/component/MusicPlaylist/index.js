...
class MusicPlaylist extends Component {
  ...
  renderNotFound = () => (
    <div className="not-found-container">
      <p className="not-found-para">No Songs Found</p>
    </div>
  )

  render() {
    const {searchInput, playList} = this.state
    const searchResults = playList.filter(eachSuggestion =>
      eachSuggestion.name.toLowerCase().includes(searchInput.toLowerCase()),
    )
    const searchListLength = searchResults.length !== 0

    return (
      <PlaylistContainer>
        <PlayerContainer>
          <PlayerHeading>Songs Playlist</PlayerHeading>
          <SearchContainer>
            <PlayerSearch
              type="search"
              placeholder="Search"
